<template>
  <Bar
    id="back"
    title="NIEF"
    color="hover:text-fuchsia-500"
  />
  <CardImage>
    <img src="~assets/images/nief.png" width="1080" height="1080" alt="NIEF Cover" />
  </CardImage>
  <CardDetail>
    <Detail name="#01">
      <div class="flex flex-1 justify-between">
        <span>QUATRE-VINGT-DIX-NEUF</span>
        <span>2:38</span>
      </div>
    </Detail>
    <Detail name="#02">
      <div class="flex flex-1 justify-between">
        <span>COWEED-19</span>
        <span>2:18</span>
      </div>
    </Detail>
    <Detail name="#03">
      <div class="flex flex-1 justify-between">
        <span>XD</span>
        <span>2:33</span>
      </div>
    </Detail>
    <Detail name="#04">
      <div class="flex flex-1 justify-between">
        <span>BANJO</span>
        <span>2:45</span>
      </div>
    </Detail>
  </CardDetail>
  <CardImage class="col-span-2">
    <div class="w-full aspect-w-16 aspect-h-9">
      <iframe class="w-full" width="560" height="315" src="https://www.youtube.com/embed/kz9_0yhU6HI" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>
    </div>
  </CardImage>
  <CardText>
    <HeadingSans>Lyrics</HeadingSans><br />
    <HeadingSerif class="text-5xl">
      QUATRE-VINGT-DIX-NEUF
    </HeadingSerif>
  </CardText>
  <CardText>
    <div class="space-y-8 text-white uppercase">
      <p>
        Quatre-vingt-dix-neuf<br />
        Quatre-vingt-dix-neuf<br />
        Quatre-vingt-dix-neuf<br />
        Quatre-vingt-dix-neuf
      </p>
      <p>
        Trois fois trente trois...<br />
        Est égal à quatre-vingt-dix-neuf<br />
        Onze fois neuf
      </p>
      <p>
        Quatre-vingt-dix-neuf<br />
        Quatre-vingt-dix-neuf<br />
        Quatre-vingt-dix-neuf<br />
        Quatre-vingt-dix-neuf
      </p>
      <p>
        As tu appris les tables de multiplication?<br />
        C’est comme pour les maths pour les raisonnements<br />
        Je fais toujours juste
      </p>
      <p>
        Mathématiques, scientifiques<br />
        Numérique, logique, analogique<br />
        Satanique
      </p>
    </div>
  </CardText>
  <CardImage class="col-span-2">
    <div class="w-full aspect-w-16 aspect-h-9">
      <iframe class="w-full" width="560" height="315" src="https://www.youtube.com/embed/nupbDm3C7-0" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>
    </div>
  </CardImage>
  <CardText>
    <HeadingSans>Lyrics</HeadingSans><br />
    <HeadingSerif class="text-5xl">
      COWEED-19
    </HeadingSerif>
  </CardText>
  <CardText>
    <div class="space-y-8 text-white uppercase" style="direction: rtl">
      <p>
        تتكيف زطلة، قارو شيرة<br />
        عمر راسك الكويد فيها
      </p>
      <p>
        نحب نخلوي روحي, ما نضر ما نآدي<br />
        نحب انزهي روحي, ما نشقا ما انباطي
      </p>
      <p>
        نكمي زطلة, قارو شيرة<br />
        خلوي راسك. الكويد فيها
      </p>
      <p>
        نحب السوسطة, جا المرض فالكرة<br />
        حاب يدينا, الكويد يخلوينا
      </p>
      <p>
        تتكيف زطلة، قارو شيرة<br />
        عمر راسك الكويد فيها
      </p>
      <p>
        انا هو الكويد, حشيش مفيد<br />
        مرض غير مميت, ليس كالكوفيد<br />
        دواء منعش, تأثير مدهش<br />
        كويد ١٩, الكوفيد ما بقاش
      </p>
      <p>
        أنا هو الكويد, حشيش لذيذ<br />
        مرض جد مريح ,ليس كالكوفيد<br />
        دواء مدهش, دواء منعش<br />
        كويد ١٩, الكوفيد ما بقاش
      </p>
      <p>
        جاء الكويد, لكويد المبيد<br />
        جاء الكويد, لكويد العنيد
      </p>
    </div>
  </CardText>
  <CardImage class="col-span-2">
    <div class="w-full aspect-w-16 aspect-h-9">
      <iframe class="w-full" width="560" height="315" src="https://www.youtube.com/embed/o0gx1hBdvCw" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>
    </div>
  </CardImage>
  <CardText>
    <HeadingSans>Lyrics</HeadingSans><br />
    <HeadingSerif class="text-5xl">
      XD
    </HeadingSerif>
  </CardText>
  <CardText>
    <div class="space-y-8 text-white uppercase">
      <p>
        Informatique, robotique, electronique<br />
        <span class="block" style="direction: rtl">كلش تمنييك,جاي لدنيا حسبت روحك انت هو ربها</span>
        Your real life, turned into fake life<br />
        Please take off the mask!<br />
        Break it in half and throw it away<br />
      </p>
      <p>
        Virus, scam, réseaux sociaux de merde<br />
        Internet, Connexion, bienvenus
      </p>
      <p>
        Je ne sais pas ce qui m'arrive<br />
        Suis je toujours vivant?<br />
        Tu Ne sais pas ce qui t’arrive<br />
        Tu te crois toujours conscient
      </p>
      <p style="direction: rtl">
        كلش تمسخير حسبت روحك حاجة,<br />
        ماعلابالكش واش راهو صاري فالحقيقة,<br />
        واش راهو صاري في هاذ الدنيا
      </p>
    </div>
  </CardText>
  <CardImage class="col-span-2">
    <div class="w-full aspect-w-16 aspect-h-9">
      <iframe class="w-full" width="560" height="315" src="https://www.youtube.com/embed/Y2Plp-j8BwM" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>
    </div>
  </CardImage>
  <CardText>
    <HeadingSans>Lyrics</HeadingSans><br />
    <HeadingSerif class="text-5xl">
      BANJO
    </HeadingSerif>
  </CardText>
  <CardText>
    <div class="space-y-8 text-white uppercase">
      <p>
        Tu viens de la terre, sorti dans l’espace<br />
        Parcourir galaxie, avec ton grand vaisseau<br />
        Voir les colonies, en jouant du banjo<br />
        Un voyage en musique, des notes en do
      </p>
      <p>
        Un astronaute, dans les colonies spatiales qui joue du
        banjo
      </p>
      <p>
        Tu navigues dans l’espace, avec ton grand vaisseau<br />
        Tu vas parfois loin, tu ne l’oublie jamais<br />
        Tu le prend ou tu vas, c’est ton banjo fétiche<br />
        Meme dans les colonies, il t’accompagne partout
      </p>
      <p>
        Un astronaute, dans les colonies spatiales qui joue du
        banjo
      </p>
      <p>
        Banjo, Banjo, Banjo, Banjo<br />
        Un, deux, trois, quatre, cinq, six, sept, huit
      </p>
      <p>
        You play the Banjo, on the right tempo<br />
        You are a hero, like a great maestro<br />
        You play in space colonies, this is your priority<br />
        Banjo maestro, banjo, banjo
      </p>
      <p>
        Colonies spatiales, tu as mis ta combinaison<br />
        Aventure, un astronaute<br />
        Des notes en sanglot<br />
        Colonies spatiales
      </p>
    </div>
  </CardText>
  <CardDetail class="col-span-2">
    <Detail name="Title">
      NIEF
    </Detail>
    <Detail name="Description">
      NIEF (EP), by XXXXXXXOOO featuring Redouane Aouameur. A Collaboration via Fiverr® Gig®. Recorded in FL Studio 20 during the COVID-19 quarantine.
    </Detail>
    <Detail name="Type">
      EP
    </Detail>
    <Detail name="Year">
      <Tag color="bg-fuchsia-500">
        2021
      </Tag>
    </Detail>
    <Detail name="Extra">
      <NavigationLink
        class="underline"
        color="hover:text-fuchsia-500"
        external="true"
        id="instagram-nief"
        to="https://instagram.com/ar/1241536719557611"
      >
        Instagram Filter
      </NavigationLink>
    </Detail>
  </CardDetail>
  <CardImage class="col-span-2">
    <div class="w-full aspect-w-16 aspect-h-9">
      <iframe class="w-full" width="560" height="315" src="https://www.youtube.com/embed/ERFJKljCXvs" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>
    </div>
  </CardImage>
  <CardMain
    class="pattern"
    color="group-hover:text-fuchsia-500"
    description="Available now!"
    external="true"
    id="applemusic-nief"
    label="Listen on"
    title="Apple Music"
    url="https://music.apple.com/br/album/nief-ep/1551229648"
  />
  <CardMain
    class="pattern"
    color="group-hover:text-fuchsia-500"
    description="Available now!"
    external="true"
    id="bandcamp-nief"
    label="Listen on"
    title="Bandcamp"
    url="https://xxxxxxxooo.bandcamp.com/album/nief"
  />
  <CardMain
    class="pattern"
    color="group-hover:text-fuchsia-500"
    description="Available now!"
    external="true"
    id="deezer-nief"
    label="Listen on"
    title="Deezer"
    url="https://deezer.com/br/album/203576412"
  />
  <CardMain
    class="pattern"
    color="group-hover:text-fuchsia-500"
    description="Available now!"
    external="true"
    id="spotify-nief"
    label="Listen on"
    title="Spotify"
    url="https://open.spotify.com/album/5gX1vki0e5muS6cZlAKRJC"
  />
  <CardMain
    class="rounded-b-none sm:rounded-bl-lg sm:rounded-br-none pattern"
    color="group-hover:text-fuchsia-500"
    description="Available now!"
    external="true"
    id="tidal-nief"
    label="Listen on"
    title="Tidal"
    url="https://tidal.com/browse/album/171478178"
  />
  <CardMain
    class="rounded-b-lg sm:rounded-bl-none sm:rounded-br-lg pattern"
    color="group-hover:text-fuchsia-500"
    description="Available now!"
    external="true"
    id="youtube-nief"
    label="Watch on"
    title="Youtube"
    url="https://youtube.com/watch?v=kz9_0yhU6HI&list=PLWjcKkvgds--JU0L2XjDhaEJbKwNZJCk6"
  />
</template>
<script>
  export default {
    mounted() {
      this.$nextTick(() => {
      })
    },
    beforeRouteLeave(to, from, next) {
      setTimeout(function() {
        next()
      }, 173)
    }
  }
</script>
